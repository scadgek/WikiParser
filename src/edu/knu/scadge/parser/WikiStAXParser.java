...
import edu.knu.scadge.semantics.DAO;
import edu.knu.scadge.semantics.Lemmatizer;
...
//              }
            ...
            else if( reader.getLocalName().equalsIgnoreCase( "title" ) )
            {
              ...
              if( articleShouldBeProcessed )
              {
                ...
                String lemma = Lemmatizer.getInstance().lemmatize( currentTitle.toString() );

                if( !lemma.isEmpty() )
                  DAO.getInstance().insertWord( lemma, true );
              }
